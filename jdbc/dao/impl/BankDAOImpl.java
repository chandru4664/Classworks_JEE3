...
public class BankDAOImpl implements BankDAO {
	...
	private final String UPDATE_ACCOUNT_SQL = "UPDATE icicibank set account_balance = ? WHERE account_no=?";
	@Override
	...
	private Account getCurrentAccountDetail(Connection connection, Long fromAccountNumber) throws SQLException{
		String GET_ACCOUNT_SQL="SELECT *FROM icicibank WHERE account_no=?";
		try(PreparedStatement ps=connection.prepareStatement(GET_ACCOUNT_SQL)){
			...
			if(rs.next()){
				...
				dBaccount.setAccountHolderName(rs.getString("account_holdername"));
